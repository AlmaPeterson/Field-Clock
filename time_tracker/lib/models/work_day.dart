class WorkDay {
  ...
  Duration get totalDurationRaw => Duration(minutes: totalMinutesRaw);
  Duration get totalDurationRounded => Duration(minutes: totalMinutesRounded);
  ...
  factory WorkDay.fromMap(Map<String, dynamic> map) => WorkDay(
    ...
    clockInTime: map['clock_in_time'] != null
        ? DateTime.parse(map['clock_in_time']) : null,
    clockInPhoto: map['clock_in_photo'],
    clockInLocation: map['clock_in_location'],
    clockOutTime: map['clock_out_time'] != null
        ? DateTime.parse(map['clock_out_time']) : null,
    clockOutPhoto: map['clock_out_photo'],
    clockOutLocation: map['clock_out_location'],
    totalMinutesRaw: map['total_minutes_raw'] ?? 0,
    totalMinutesRounded: map['total_minutes_rounded'] ?? 0,
  );

  WorkDay copyWith({
    int? id, int? jobId,
    DateTime? clockInTime, String? clockInPhoto, String? clockInLocation,
    DateTime? clockOutTime, String? clockOutPhoto, String? clockOutLocation,
    int? totalMinutesRaw, int? totalMinutesRounded,
  }) => WorkDay(
    id: id ?? this.id,
    ...
    totalMinutesRaw: totalMinutesRaw ?? this.totalMinutesRaw,
    totalMinutesRounded: totalMinutesRounded ?? this.totalMinutesRounded,
  );
}